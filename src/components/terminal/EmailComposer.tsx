...
export function EmailComposer() {
  ...
  return (
    <div className="h-full overflow-y-auto term-scroll p-3 sm:p-5" onClick={() => {
      ...
      <div className="text-term-accent text-glow mb-3">compose — terminal email</div>
      <div className="space-y-2 text-sm">
        <label className="flex items-baseline gap-2">
          <span className="text-term-cyan w-20 shrink-0">To:</span>
          <input
            ref={toRef}
            value={to}
            onChange={(e) => setTo(e.target.value)}
            onFocus={() => setFocusIdx(0)}
            className="flex-1 bg-transparent border-none outline-none text-term-fg caret-term-accent"
            spellCheck={false}
          />
          {field(0)}
        </label>
        <label className="flex items-baseline gap-2">
          <span className="text-term-cyan w-20 shrink-0">Subject:</span>
          <input
            ref={subjectRef}
            value={subject}
        ...
        <label className="flex flex-col gap-1">
          ...
          <textarea
            ...
            rows={8}
            className="w-full bg-term-soft border border-term-dim p-2 outline-none text-term-fg caret-term-accent resize-none term-scroll"
            spellCheck={false}
          />
        </label>
        <div className="text-term-muted text-xs pt-1">
